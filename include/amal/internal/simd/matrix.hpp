...
namespace amal::internal
{
    ...
    {
        __m128i t0 = _mm_unpacklo_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(t0, t0)); // a0 b0
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(t0, t0)); // a1 b1
    }

    inline void transpose(__v4si_u const (&in)[3], __v4si_u (&out)[2])
    {
        __m128i t0 = _mm_unpacklo_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        __m128i t1 = _mm_unpacklo_epi32(AMAL_M128I(in[2]), _mm_setzero_si128());
        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(t0, t1));
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(t0, t1));
    }

    inline void transpose(__v4si_u const (&in)[2], __v4si_u (&out)[3])
    {
        __m128i t0 = _mm_unpacklo_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        __m128i t1 = _mm_unpackhi_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(t0, t0));
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(t0, t0));
        out[2] = AMAL_V4SI(_mm_unpacklo_epi64(t1, t1));
    }
    ...
    inline void transpose(__v4si_u const (&in)[3], __v4si_u (&out)[3])
    {
        ...
        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(t0, t0));
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(t0, t0));
        out[2] = AMAL_V4SI(_mm_unpacklo_epi64(t1, AMAL_M128I(in[2])));
    }

    inline void transpose(__v4si_u const (&in)[4], __v4si_u (&out)[2])
    {
        __m128i ab_lo = _mm_unpacklo_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        __m128i cd_lo = _mm_unpacklo_epi32(AMAL_M128I(in[2]), AMAL_M128I(in[3]));

        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(ab_lo, cd_lo));
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(ab_lo, cd_lo));
    }

    inline void transpose(__v4si_u const (&in)[2], __v4si_u (&out)[4])
    {
        __m128i lo = _mm_unpacklo_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        __m128i hi = _mm_unpackhi_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));

        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(lo, lo));
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(lo, lo));
        out[2] = AMAL_V4SI(_mm_unpacklo_epi64(hi, hi));
        out[3] = AMAL_V4SI(_mm_unpackhi_epi64(hi, hi));
    }

    inline void transpose(__v4si_u const (&in)[3], __v4si_u (&out)[4])
    {
        __m128i ab_lo = _mm_unpacklo_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        __m128i ab_hi = _mm_unpackhi_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        __m128i c_lo = _mm_unpacklo_epi32(AMAL_M128I(in[2]), _mm_setzero_si128());
        __m128i c_hi = _mm_unpackhi_epi32(AMAL_M128I(in[2]), _mm_setzero_si128());

        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(ab_lo, c_lo));
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(ab_lo, c_lo));
        out[2] = AMAL_V4SI(_mm_unpacklo_epi64(ab_hi, c_hi));
        out[3] = AMAL_V4SI(_mm_unpackhi_epi64(ab_hi, c_hi));
    }
    ...
    inline void transpose(__v4si_u const (&in)[4], __v4si_u (&out)[3])
    {
        ...
        __m128i ab_hi = _mm_unpackhi_epi32(AMAL_M128I(in[0]), AMAL_M128I(in[1]));
        __m128i cd_lo = _mm_unpacklo_epi32(AMAL_M128I(in[2]), AMAL_M128I(in[3]));
        __m128i cd_hi = _mm_unpackhi_epi32(AMAL_M128I(in[2]), AMAL_M128I(in[3]));

        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(ab_lo, cd_lo));
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(ab_lo, cd_lo));
        out[2] = AMAL_V4SI(_mm_unpacklo_epi64(ab_hi, cd_hi));
    }
    ...
    inline void transpose(__v4si_u const (&in)[4], __v4si_u (&out)[4])
    {
        ...
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(ab_lo, cd_lo));
        out[2] = AMAL_V4SI(_mm_unpacklo_epi64(ab_hi, cd_hi));
        out[3] = AMAL_V4SI(_mm_unpackhi_epi64(ab_hi, cd_hi));
    }

    template <>
    inline void transpose(__v4si_u const (&in)[4], __v4si_u (&out)[4])
    {
        __m128i a = AMAL_M128I(in[0]);
        __m128i b = AMAL_M128I(in[1]);
        __m128i c = AMAL_M128I(in[2]);
        __m128i d = AMAL_M128I(in[3]);
        ...
        out[0] = AMAL_V4SI(_mm_unpacklo_epi64(ab_lo, cd_lo)); // a0 b0 c0 d0
        out[1] = AMAL_V4SI(_mm_unpackhi_epi64(ab_lo, cd_lo)); // a1 b1 c1 d1
        out[2] = AMAL_V4SI(_mm_unpacklo_epi64(ab_hi, cd_hi)); // a2 b2 c2 d2
        out[3] = AMAL_V4SI(_mm_unpackhi_epi64(ab_hi, cd_hi)); // a3 b3 c3 d3
    }

    inline __v4si_u determinant(__v4si_u const (&m)[2])
    {
        __v4si_u ab =
            mm_mullo_epi32_compat(m[0], AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[1]), _MM_SHUFFLE(0, 0, 1, 1))));
        __m128i ab_i = AMAL_M128I(ab);
        return AMAL_V4SI(_mm_sub_epi32(ab_i, _mm_shuffle_epi32(ab_i, _MM_SHUFFLE(1, 1, 1, 1)))); // .x = det
    }
    ...
    inline __v4si_u determinant(__v4si_u const (&m)[3])
    {
        ...
        __v4si_u bc = mm_mullo_epi32_compat(AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(b), _MM_SHUFFLE(1, 2, 0, 1))),
                                            AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(c), _MM_SHUFFLE(2, 0, 1, 2))));
        __v4si_u cb = mm_mullo_epi32_compat(AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(b), _MM_SHUFFLE(2, 0, 1, 2))),
                                            AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(c), _MM_SHUFFLE(1, 2, 0, 1))));

        __v4si_u cross = AMAL_V4SI(_mm_sub_epi32(AMAL_M128I(bc), AMAL_M128I(cb)));
        __v4si_u det = mm_mullo_epi32_compat(a, cross);

        __m128i det_i = AMAL_M128I(det);
        return AMAL_V4SI(_mm_add_epi32(_mm_shuffle_epi32(det_i, _MM_SHUFFLE(0, 0, 0, 0)),
                                       _mm_add_epi32(_mm_shuffle_epi32(det_i, _MM_SHUFFLE(1, 1, 1, 1)),
                                                     _mm_shuffle_epi32(det_i, _MM_SHUFFLE(2, 2, 2, 2)))));
    }

    inline __v4si_u determinant(__v4si_u const (&m)[4])
    {
        // s0 = (m2[0,1,1,2] * m3[3,2,3,3] - m2[3,2,3,3] * m3[0,1,1,2]) * m1[0,0,0,1]
        __v4si_u t0 = mm_mullo_epi32_compat(AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[2]), _MM_SHUFFLE(0, 1, 1, 2))),
                                            AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[3]), _MM_SHUFFLE(3, 2, 3, 3))));
        __v4si_u t1 = mm_mullo_epi32_compat(AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[2]), _MM_SHUFFLE(3, 2, 3, 3))),
                                            AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[3]), _MM_SHUFFLE(0, 1, 1, 2))));
        __v4si_u diff = AMAL_V4SI(_mm_sub_epi32(AMAL_M128I(t0), AMAL_M128I(t1)));
        __v4si_u s0 =
            mm_mullo_epi32_compat(diff, AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[1]), _MM_SHUFFLE(0, 0, 0, 1))));

        // prod = m2[0,0,1,2] * m3[1,2,0,0]
        __v4si_u prod = mm_mullo_epi32_compat(AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[2]), _MM_SHUFFLE(0, 0, 1, 2))),
                                              AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[3]), _MM_SHUFFLE(1, 2, 0, 0))));
        __v4si_u upper =
            AMAL_V4SI(_mm_castps_si128(_mm_movehl_ps(_mm_setzero_ps(), _mm_castsi128_ps(AMAL_M128I(prod)))));

        __v4si_u s1 = mm_mullo_epi32_compat(AMAL_V4SI(_mm_sub_epi32(AMAL_M128I(prod), AMAL_M128I(upper))),
                                            AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(m[1]), _MM_SHUFFLE(2, 3, 3, 3))));

        __v4si_u det = AMAL_V4SI(_mm_add_epi32(AMAL_M128I(s0), AMAL_M128I(s1)));
        __v4si_u signs = AMAL_V4SI(_mm_setr_epi32(1, -1, 1, -1));
    ...
    inline void transpose(__m256d_u const (&in)[3], __m256d_u (&out)[3])
    {
        ...
        out[2] = _mm256_blend_pd(_mm256_castpd128_pd256(_mm256_castpd256_pd128(in[2])),
