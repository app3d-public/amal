...
namespace amal::internal
{
    ...
    inline int extract_scalar(__v4si_u v) { return _mm_cvtsi128_si32(AMAL_M128I(v)); }
    ...
    inline __v4si_u min(__v4si_u a, __v4si_u b) { return AMAL_V4SI(_mm_min_epi32(AMAL_M128I(a), AMAL_M128I(b))); }

    inline __v4si_u max(__v4si_u a, __v4si_u b) { return AMAL_V4SI(_mm_max_epi32(AMAL_M128I(a), AMAL_M128I(b))); }
    #else
    ...
    {
        __v4si_u mask = AMAL_V4SI(_mm_cmplt_epi32(AMAL_M128I(a), AMAL_M128I(b)));
        return (a & mask) | (b & ~mask);
    }

    inline __v4si_u max(__v4si_u a, __v4si_u b)
    {
        __v4si_u mask = AMAL_V4SI(_mm_cmpgt_epi32(AMAL_M128I(a), AMAL_M128I(b)));
        return (a & mask) | (b & ~mask);
    }
    #endif
#endif
    ...
    #if defined(__AVX2__)
        const int s = _MM_SHUFFLE(c, c, c, c);
        return _mm256_permute4x64_pd(x, s);
    #else
        constexpr int lane = (c >= 2) ? 1 : 0;
        constexpr int idx = c & 1;
        const int s = idx ? 0b1111 : 0b0000;
        __m256d_u within = _mm256_permute_pd(x, s);
        return _mm256_permute2f128_pd(within, within, lane ? 0x11 : 0x00);
    #endif
    }
    ...
    inline __m256d_u round(__m256d_u x) { return _mm256_round_pd(x, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC); }
