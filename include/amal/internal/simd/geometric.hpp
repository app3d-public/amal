...
namespace amal::internal
{
    ...
    {
        __v4si_u a_yzx = AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(a), _MM_SHUFFLE(3, 0, 2, 1)));
        __v4si_u b_yzx = AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(b), _MM_SHUFFLE(3, 1, 0, 2)));
        __v4si_u a_zxy = AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(a), _MM_SHUFFLE(3, 1, 0, 2)));
        __v4si_u b_zxy = AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(b), _MM_SHUFFLE(3, 0, 2, 1)));
        return mm_mullo_epi32_compat(a_yzx, b_yzx) - mm_mullo_epi32_compat(a_zxy, b_zxy);
    }

    inline __v4si_u cross(__v4si_u a, __v4si_u b)
    {
        return AMAL_V4SI(_mm_shuffle_epi32(AMAL_M128I(cross_yzx(a, b)), _MM_SHUFFLE(3, 0, 2, 1)));
    }
    ...
    inline __v4si_u dot(__v4si_u a, __v4si_u b)
    {
        ...
        __m128i mul_i = AMAL_M128I(mul);
        __m128i temp_i = _mm_add_epi32(mul_i, _mm_shuffle_epi32(mul_i, _MM_SHUFFLE(2, 2, 0, 1)));
        int sum = _mm_cvtsi128_si32(temp_i) + _mm_extract_epi32(temp_i, 1);
    #else
        __m128i mul_i = AMAL_M128I(mul);
        __m128i t1 = _mm_add_epi32(mul_i, _mm_srli_si128(mul_i, 8));
        __m128i t2 = _mm_add_epi32(t1, _mm_srli_si128(t1, 4));
        int sum = _mm_cvtsi128_si32(t2);
    #endif
        return AMAL_V4SI(_mm_set1_epi32(sum));
    }
