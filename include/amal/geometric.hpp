...
namespace amal
{
    ...
    inline constexpr AMAL_TYPE_SIMD(AMAL_NVEC(N), T) dot(AMAL_NVEC(N) const &v1, AMAL_NVEC(N) const &v2)
    {
        static_assert(is_floating_point_v<T>, "Dot product only valid for floating-point types");
        return details::extract_scalar(details::dot(v1.s, v2.s));
    }
    ...
    {
        return details::sqrt(details::dot(v.s, v.s));
    }
    ...
    {
        return AMAL_NVEC(3)(details::cross(v1.s, v2.s));
    }
    ...
    inline AMAL_VEC_VAL_SIMD normalize(AMAL_VEC_SELF const &v)
    {
        ...
        return AMAL_VEC_SELF(v.s * details::inverse_sqrt(details::dot(v.s, v.s)));
    }
